...
const RebootAllConfirmationModal = ({ isOpen, onClose, onConfirm, deviceCount, isLoading }) => {
			...
			<div className={`${colors.card} rounded-xl max-w-md w-full p-6 shadow-xl`}>
				...
				<div className="mb-6">
					<p className={`${colors.textSecondary} mb-4`}>
						Are you sure you want to reboot all LUXEGENIE's ?
					</p>
					...
					<p className={`text-sm ${colors.textMuted} mt-3`}>
						All devices will restart and may be temporarily unavailable.
					</p>
				</div>
				...
				<div className="flex space-x-3">
					...
					>
						{isLoading ? "Rebooting..." : "Yes, Reboot All"}
					</button>
				</div>
			</div>
		</div>
...
export default RebootAllConfirmationModal;
