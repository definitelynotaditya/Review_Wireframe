...
const UpdateChefSpecialModal = ({ isOpen, onClose, restaurantId, chefSpecial }) => {
	...
	// Effect to populate form when chefSpecial data is provided
	useEffect(() => {
		if (chefSpecial && isOpen) {
			console.log("UpdateChefSpecialModal received chefSpecial:", chefSpecial);
			setFormData({
				dish_name: chefSpecial.dish_name || "",
				dish_description: chefSpecial.dish_description || "",
				img_name: chefSpecial.img_name || chefSpecial.dish_img || "", // Support both field names
				...
				active: typeof chefSpecial.active === "boolean" ? chefSpecial.active : true,
				newImageFile: null
			});
			
			// Handle image preview - use dish_img directly as it contains CloudFront URL
			const imageUrl = chefSpecial.dish_img || chefSpecial.img_name || "";
			setPreviewImage(imageUrl);
		}
	}, [chefSpecial, isOpen]);

	// Mutation for updating chef special
	const updateChefSpecialMutation = useMutation({
		mutationFn: (chefSpecialData) => {
			if (!chefSpecial?.chef_special_id) {
				throw new Error("Chef special ID is missing");
			}
			// console.log("Updating chef special with ID:", chefSpecial.chef_special_id);
			// console.log("Update data:", chefSpecialData);
			return chefSpecialsApi.saveChefSpecial(restaurantId, chefSpecialData, true, chefSpecial.chef_special_id);
		},
		onSuccess: () => {
		...
		onError: (error) => {
			console.error("Update chef special failed:", error);
			const errorMessage = error.response?.data?.message || error.message || "Failed to update chef special";
	...
	const handleSubmit = async (e) => {
		...
		console.log("Chef special being updated:", chefSpecial);
		
		setIsSubmitting(true);
		
		try {
			// Only send img_name if a new image was uploaded, otherwise send empty string
			const dataToSubmit = {
				...formData,
				img_name: formData.newImageFile ? formData.img_name : ""
			};
			await updateChefSpecialMutation.mutateAsync(dataToSubmit);
		} catch (error) {
...
export default UpdateChefSpecialModal;