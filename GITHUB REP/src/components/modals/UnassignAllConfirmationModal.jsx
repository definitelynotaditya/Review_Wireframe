...
const UnassignAllConfirmationModal = ({ isOpen, onClose, onConfirm, deviceCount, isLoading }) => {
			...
			<div className={`${colors.card} rounded-xl max-w-md w-full p-6 shadow-xl`}>
				...
				<div className="mb-6">
					<p className={`${colors.textSecondary} mb-4`}>
						Are you sure you want to unassign all LUXEGENIE's from their tables?
					</p>
					...
					<p className={`text-sm ${colors.textMuted} mt-3`}>
						This action will remove all device assignments from their tables.
					</p>
				</div>
				...
				<div className="flex space-x-3">
					...
					>
						{isLoading ? "Unassigning..." : "Yes, Unassign All"}
					</button>
				</div>
			</div>
		</div>
...
export default UnassignAllConfirmationModal;
