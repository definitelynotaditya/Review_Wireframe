...
const UnassignLuxegenieModal = ({ isOpen, onClose, onConfirm, device, isLoading }) => {
			...
			<div className={`${colors.card} rounded-xl max-w-md w-full p-6 shadow-xl`}>
				...
				<div className="mb-6">
					<p className={`${colors.textSecondary} mb-4`}>
						Are you sure you want to unassign this LUXEGENIE device?
					</p>
					
					{device && (
						<div className={`${colors.secondary} rounded-lg p-4 space-y-2`}>
							<div className="flex justify-between">
								<span className={`text-sm font-medium ${colors.textSecondary}`}>Device ID:</span>
								<span className={`text-sm ${colors.textPrimary}`}>#{device.device_id}</span>
							</div>
							<div className="flex justify-between">
								<span className={`text-sm font-medium ${colors.textSecondary}`}>Serial Number:</span>
								<span className={`text-sm ${colors.textPrimary}`}>{device.serial_number}</span>
							</div>
							<div className="flex justify-between">
								<span className={`text-sm font-medium ${colors.textSecondary}`}>Currently Assigned To:</span>
								<span className={`text-sm ${colors.textPrimary}`}>
									Table {device.assigned_to_table_no} ({device.assigned_to_sitting_area})
								</span>
							</div>
							{device.table_id && (
								<div className="flex justify-between">
									<span className={`text-sm font-medium ${colors.textSecondary}`}>Table ID:</span>
									<span className={`text-sm ${colors.textPrimary}`}>{device.table_id}</span>
								</div>
							)}
						</div>
					)}
					...
					<p className={`text-sm ${colors.textMuted} mt-3`}>
						This action will remove the device assignment from the table.
					</p>
				</div>
...
export default UnassignLuxegenieModal;