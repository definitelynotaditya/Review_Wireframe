...
const AddUserModal = ({ isOpen, onClose, restaurantId }) => {
	...
	const addUserMutation = useMutation({
		...
		onError: (error) => {
			...
			if (error.response?.data?.error === "Server code already exists") {
				toast.error(
					"Server code already exists. Please use a different server code."
				);
			} else {
				toast.error(
	...
	const handleServerCodeBlur = async () => {
		// Only check if server code is not empty
		if (!formData.server_code) {
			setServerCodeError("");
			return;
		}
		...
		try {
			const result = await userApi.checkServerCode(
				restaurantId,
				formData.server_code
			);

			if (result.isTaken) {
				setServerCodeError(
					result.msg || "Server code is already in use"
				);
			} else {
				setServerCodeError("");
			}
		} catch (error) {
			console.error("Error checking server code:", error);
			// Don't show error to user if the check fails
			setServerCodeError("");
		} finally {
	...
	const validateForm = () => {
		if (serverCodeError) {
			toast.error("Please use a different server code");
			return false;
		}
	...
	const handleSubmit = async (e) => {
		...
		const userData = {
			...
			role: formData.role,
			img_name: formData.img_name, // Include uploaded image filename
		};
	...
	return (
		<>
			...
			<div className="fixed inset-0 bg-black/80 bg-opacity-50 flex items-center justify-center z-50 ">
				<div
					className={`${colors.card} rounded-2xl p-8 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto scrollbar-none`}
				>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div>
						<label
							...
							<input
								...
								onBlur={handleServerCodeBlur}
								className={`w-full px-4 py-3 ${colors.input} border ${
									serverCodeError
										? "border-red-500"
										: colors.border
					...
					<div
						className={`flex justify-end space-x-4 pt-6 border-t ${colors.border}`}
					>
						<button
							type="button"
							onClick={handleClose}
